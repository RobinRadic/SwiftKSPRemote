...
namespace Radical.KerbalApiServer.UI
{
    using Extensions;
    ...
    public class Window : MonoBehaviour
    {
        ...
        internal static GameObject GameObject
        {
            get
            {
                if (_gameObject == null)
                {
                    _gameObject = new GameObject("SKRWindowManager");
        ...
        private static ConfigNode WindowPos { get { return Config.WindowPositions; } }
        ...
        public static void Create(string title, bool savepos, bool ensureUniqueTitle, int width, int height, Action<Window> drawFunc)
        {
            ...
            if (ensureUniqueTitle && allOpenWindows.Any(w => w._title == title))
            {
                ...
                if (title == "Code Compiler")
                {
                    Utils.Log("Code Compiler!");
                    foreach (var v in allOpenWindows.Where(p => (p._title == title)))
                    {
                        v.DrawWindow(v.GetInstanceID());
                    }
                }
