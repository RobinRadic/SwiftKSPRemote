       ...
       // private const string _tooltipOn = "Hide Notepad";
            ...
            if (GUILayout.Button("Change Dir"))
            {
                if (_dirs.Count == 0)
                {
                    _notesDir = KSPUtil.ApplicationRootPath.Replace("\\", "/") +
                                "GameData/KerbalApiServer/Plugins/PluginData/KerbalApiServer/";
                    _fileNames = null;
                    _showList = false;
                    GetNotes();
                    _showList = true;
        ...
        private void LoadSettings()
        {
            ...
            _notesDir = _configFile.GetValue("notesdir", KSPUtil.ApplicationRootPath.Replace("\\", "/") +
                                                        "GameData/KerbalApiServer/PluginData/KerbalApiServer/");
        ...
        public CodeCompiler compiler
        {
            get
            {
                var c = gameObject.GetComponent<CodeCompiler>();
                return c == null ? gameObject.AddComponent<CodeCompiler>() : c;
            }
        }
        private void NotesWindow(int windowId)
        {
            ...
            // Opens the notes list windows.
            if (GUI.Button(new Rect(385f, 410f, 80f, 30f), "Compile"))
            {
                compiler.RunCode(_text);
            }
